    ...
    auto process_blocks = [=](size_t block_start, size_t block_end) {
        for (size_t n_block = block_start; n_block < block_end; ++n_block) {
            ...
            for (; g + 1 < num_groups; g += 2) {
                ...
                const size_t k_base0 = g * group_size;
                const size_t k_base1 = k_base0 + group_size;
                const int8_t* b0 = B + (n_block * K + k_base0) * 4;
                const int8_t* b1 = B + (n_block * K + k_base1) * 4;
                ...
                float32x4_t s1 = vcvt_f32_f16(vld1_f16(B_scales + (n_block * num_groups + g + 1) * 4));
