...
// Single-threaded i8mm INT8 GEMV (no internal pool dispatch). Same math as
// cactus_gemv_int8_i8mm. For use inside an outer parallel loop.
void cactus_gemv_int8_i8mm_st(
...
// I8MM variant of cactus_gemv_int8_active_block_runs: processes only the
// listed N-block runs (selective output rows). Used for up_proj in the K96
// grouped MLP. Mirrors the inner loop of cactus_gemv_int8_i8mm. Output rows
// not covered by any run are LEFT UNTOUCHED.
void cactus_gemv_int8_active_block_runs_i8mm(
    ...
    static thread_local std::vector<uint16_t> flat_blocks_i8mm_up;
    flat_blocks_i8mm_up.clear();
    size_t total_blocks = 0;
    for (size_t r = 0; r < num_runs; ++r) total_blocks += runs[2 * r + 1];
    flat_blocks_i8mm_up.reserve(total_blocks);
    for (size_t r = 0; r < num_runs; ++r) {
        uint16_t s = runs[2 * r];
        uint16_t c = runs[2 * r + 1];
        for (uint16_t i = 0; i < c; ++i) flat_blocks_i8mm_up.push_back(s + i);
    }
    const uint16_t* flat = flat_blocks_i8mm_up.data();
...
// I8MM variant of cactus_gemv_int8_active_kgroup_runs: processes only the
// listed K-group runs (selective input dims). Used for down_proj in the K96
// grouped MLP. All N output positions are written. Mirrors the inner loop
// of cactus_gemv_int8_i8mm.
