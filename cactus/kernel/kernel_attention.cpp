...
static inline void cactus_attention_f16_fast(
    ...
    CactusThreading::parallel_for(batch_size * num_q_heads * seq_len, CactusThreading::Thresholds::ATTENTION,
        ...
        std::vector<float16x8_t> vacc(nblocks);
                    ...
                    for (size_t kv_idx = 0; kv_idx < block_size; ++kv_idx) {
                        ...
                        if (kv_pos < cache_len) {
                            ...
                            for (size_t quant_group = 0; quant_group < num_quant_groups; quant_group++) {
                                ...
                                float32x4_t raw_lo = vdupq_n_f32(0.0f);
                                float32x4_t raw_hi = vdupq_n_f32(0.0f);
                                ...
                                for (size_t i = 0; i < 4; i++) {
                                    ...
                                    int16x8_t k_vec_i16 = vmovl_s8(vld1_s8(&k_vec[dim_block]));

                                    raw_lo = vfmaq_f32(raw_lo, q_low, vcvtq_f32_s32(vmovl_s16(vget_low_s16(k_vec_i16))));
                                    raw_hi = vfmaq_f32(raw_hi, q_high, vcvtq_f32_s32(vmovl_s16(vget_high_s16(k_vec_i16))));
                                }

                                score += k_scale_base[quant_group] * vaddvq_f32(vaddq_f32(raw_lo, raw_hi));
                            }
                        } else {
                            const size_t new_pos = kv_pos - cache_len;
                            const __fp16* k_vec = K_new_base + new_pos * kv_seq_stride + kv_head_idx * head_dim;

                            float32x4_t score_lo = vdupq_n_f32(0.0f);
                            float32x4_t score_hi = vdupq_n_f32(0.0f);

                            for (size_t dim_block = 0; dim_block < head_dim_aligned; dim_block += VECTOR_WIDTH) {
                                float16x8_t q_vec_f16 = vld1q_f16(&q_vec[dim_block]);
                                float16x8_t k_vec_f16 = vld1q_f16(&k_vec[dim_block]);

                                score_lo = vfmaq_f32(score_lo, vcvt_f32_f16(vget_low_f16(q_vec_f16)), vcvt_f32_f16(vget_low_f16(k_vec_f16)));
                                score_hi = vfmaq_f32(score_hi, vcvt_f32_f16(vget_high_f16(q_vec_f16)), vcvt_f32_f16(vget_high_f16(k_vec_f16)));
                            }

                            score = vaddvq_f32(vaddq_f32(score_lo, score_hi));
                        }
