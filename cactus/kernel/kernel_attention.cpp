...
RoPECacheF16& precompute_rope_tables_f16(size_t seq_len, size_t head_dim, float theta) {
    RoPECacheF16* cache = nullptr;
    for (auto& candidate : rope_caches_f16) {
        if (candidate.initialized &&
            candidate.head_dim == head_dim &&
    ...
    if (cache->initialized && cache->max_seq_len >= seq_len) {
        rope_cache_f16 = *cache;
        return *cache;
    }

    const size_t half_dim = head_dim / 2;
    const bool same_shape = cache->initialized;
    const size_t old_seq_len = same_shape ? cache->max_seq_len : 0;

    if (!same_shape) {
        cache->inv_freq.resize(half_dim);
        for (size_t i = 0; i < half_dim; ++i) {
            cache->inv_freq[i] = 1.0f / powf(theta, (2.0f * i) / head_dim);
        }
        cache->cos_table.clear();
        cache->sin_table.clear();
    }
    ...
    for (size_t pos = old_seq_len; pos < seq_len; ++pos) {
        const float pos_float = static_cast<float>(pos);
        for (size_t i = 0; i < half_dim; ++i) {
            const float angle = pos_float * cache->inv_freq[i];
...
void cactus_rope_f16(
    ...
    auto& rope_cache = CactusRoPEF16::precompute_rope_tables_f16(seq_len + start_pos, head_dim, theta);
    
    const __fp16* cos_cache = rope_cache.cos_table.data() + start_pos * half_dim;
    const __fp16* sin_cache = rope_cache.sin_table.data() + start_pos * half_dim;
...
void cactus_gpt_j_rope_f16(
    ...
    auto& rope_cache = CactusRoPEF16::precompute_rope_tables_f16(seq_len + start_pos, rot_dim, theta);
    
    const __fp16* cos_cache = rope_cache.cos_table.data() + start_pos * half_rot_dim;
    const __fp16* sin_cache = rope_cache.sin_table.data() + start_pos * half_rot_dim;
