...
size_t Gemma4Model::build_attention(CactusGraph* gb, size_t input, uint32_t layer_idx,
    ...
    if (use_cache && !kv_cache_.is_empty()) {
        if (kv_cache_.quant_method == KVQuantMethod::TURBOQUANT) {
            attn = gb->attention_turboquant_hybrid(
                q4, k4, v4, attention_scale_, position_offset,
                kv_cache_.get_tq_key_radii(cache_src),
                kv_cache_.get_tq_key_angles(cache_src),
                kv_cache_.get_tq_val_radii(cache_src),
                kv_cache_.get_tq_val_angles(cache_src),
