...
int cactus_complete_with_images(
    cactus_model_t model,
    const char* messages_json,
    ...
    try {
        ...
        std::vector<std::string> image_identifiers;
        auto messages = parse_messages_json(messages_json, image_identifiers);
        ...
        // Construct MediaInput
        std::vector<Model::MediaInput> media_inputs;
        media_inputs.reserve(image_identifiers.size());

        for(const auto& id : image_identifiers) {
            bool found_buffer = false;
            for(size_t i=0; i<image_count; ++i) {
                if(images[i].id && id == images[i].id) {
                    Model::MediaInput input;
                    input.type = Model::MediaInput::Type::BUFFER;
                    input.buffer = static_cast<const unsigned char*>(images[i].data);
                    input.width = images[i].width;
                    input.height = images[i].height;
                    input.channels = images[i].channels;
                    input.id = id;
                    media_inputs.push_back(input);
                    found_buffer = true;
                    break;
                }
            }
            if(!found_buffer) {
                // Treat as path
                Model::MediaInput input;
                input.type = Model::MediaInput::Type::PATH;
                input.path = id;
                media_inputs.push_back(input);
            }
        }

        // ... (RAG context retrieval logic same as cactus_complete) ...
         if (handle->corpus_index) {
            std::string query;
            for (auto it = messages.rbegin(); it != messages.rend(); ++it) {
                if (it->role == "user") {
        ...
        bool has_images = !image_identifiers.empty();
        bool is_prefix = !has_images &&
                         (current_prompt_tokens.size() >= handle->processed_tokens.size()) &&
                         std::equal(handle->processed_tokens.begin(), handle->processed_tokens.end(), current_prompt_tokens.begin());
        ...
        if (tokens_to_process.empty()) {
            ...
            // Use decode_with_media if images are present
            if (!media_inputs.empty()) {
                next_token = handle->model->decode_with_media(tokens_to_process, media_inputs, temperature, top_p, top_k, "", &first_token_entropy);
            } else {
                size_t prefill_chunk_size = handle->model->get_prefill_chunk_size();
        ...
        // ... (Generation loop same as incomplete, omitted for brevity but should be there) ...
        // Simplification: We assume the rest of the generation loop is identical. 
        // For the sake of this edit, I will include the loop logic to be complete.
