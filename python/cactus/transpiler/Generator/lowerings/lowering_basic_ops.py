...
def is_logits_projection(node: IRModels.Node, output_width: int) -> bool:
    ...
    if output_width >= 100000 or "lm_head" in node_context_text(node).lower():
        return True
    # Only inspect weight/bias operands. Walking through the activation input
    # reaches tied token embeddings and falsely labels nearby projections as
    # the LM head (LFM-MoE's first conv projection is one such case).
    return any("lm_head" in node_context_text(parent).lower() for parent in node.parents[1:])
