...
def find_whisper_cross_key_values(graph: IRModels.Graph) -> tuple[tuple[str, str], ...]:
    ...
    for node in graph.nodes:
        ...
        key_shape, value_shape = tensor_shape(key_node), tensor_shape(value_node)
        if (
            key_node.target in {"aten.cat.default", "cactus.cat"}
            and value_node.target in {"aten.cat.default", "cactus.cat"}
            and key_shape == value_shape
            and len(key_shape) == 4
            and key_shape[2] == 1500
        ):
            pairs.append((
...
def direct_whisper_cross_kv_source(node: IRModels.Node) -> IRModels.Node:
    if node.target not in {"aten.cat.default", "cactus.cat"} or len(node.parents) != 2:
        return node
    left, right = node.parents
    if tensor_shape(left) != [0] or right.target not in {"aten.permute.default", "aten.transpose.int", "cactus.transpose"} or not right.parents:
        return node
    source = right.parents[0]
    source_shape = tensor_shape(source)
    if len(source_shape) == 4 and source_shape[0] == 1 and source_shape[1] == 1500:
        return source
    return node
...
def find_lfm_vlm_text_inputs_embeds(graph: IRModels.Graph) -> str:
    for node in graph.nodes:
        ...
        if tensor_rank(node) == 3 and shape[0] == 1:
            return node.name
