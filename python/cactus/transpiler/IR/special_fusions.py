...
LAYOUT_TARGETS = {
    ...
    "cactus.transpose", "aten.permute.default", "aten.transpose.int",
}
ATTENTION_WRAPPER_TARGETS = LAYOUT_TARGETS | {
...
def match_generic_cached_attention(
    ...
    return models.FusionResult.from_match(
        ...
        external_inputs=external_inputs,
        attrs={
            # Apply the complete exported QK scale inside native attention.
            # This avoids rounding a separately scaled query to FP16 before
            # the kernel's FP32 score accumulation.
...
def decomposed_attention_scale(qk_bmm: models.Node) -> float:
    ...
    for parent in qk_bmm.parents[:2]:
        scale *= decomposed_attention_path_scale(parent)
    return scale

def decomposed_attention_path_scale(node: models.Node, max_depth: int = 10) -> float:
    scale = 1.0
    current = node
    for _ in range(max_depth):
        if current.target in {"cactus.scalar_multiply", "aten.mul.Scalar"}:
            ...
            if value is not None:
                scale *= value
        if len(current.parents) != 1 or current.target not in {
            "cactus.view", "aten.view.default", "aten.reshape.default",
...
def whisper_value_bmm_from_output(source: models.Node, max_depth: int = 10) -> tuple[models.Node | None, tuple[models.Node, ...]]:
    if source.target not in LAYOUT_TARGETS:
        return None, ()
    return value_bmm_from_unary_path(source, max_depth)
...
def generic_unscaled_bqhd_attention_input(node: models.Node, max_depth: int = 12) -> models.Node | None:
    query = whisper_bqhd_attention_input(node, max_depth=max_depth)
    if query is None:
        return None
    shape = models.tensor_shape(query)
    current = query
    for _ in range(max_depth):
        if current.target not in {
            "cactus.scalar_multiply", "aten.mul.Scalar",
            "cactus.precision_cast", "aten._to_copy.default",
        } or len(current.parents) != 1:
            break
        parent = current.parents[0]
        if models.tensor_shape(parent) != shape:
            break
        current = parent
    return current
...
def value_bmm_from_unary_path(
    source: models.Node,
    max_depth: int,
    wrapper_targets: set[str] = ATTENTION_WRAPPER_TARGETS,
) -> tuple[models.Node | None, tuple[models.Node, ...]]:
    current = source
    ...
    for _ in range(max_depth):
        ...
        if parent.target not in wrapper_targets:
            return None, ()
        matched_nodes.append(parent)
        current = parent
    return None, ()
...
def gemma4_vision_value_bmm_from_output(source: models.Node, max_depth: int = 8) -> tuple[models.Node | None, tuple[models.Node, ...]]:
    if source.target not in {"cactus.view", "aten.view.default", "aten.reshape.default"} or node_rank(source) != 3:
        return None, ()
    return value_bmm_from_unary_path(
        source, max_depth,
        LAYOUT_TARGETS | {"cactus.precision_cast", "aten._to_copy.default"},
    )
...
def gemma4_decode_attention_core_output_match(
    ...
    if source.target not in {"cactus.view", "aten.view.default", "aten.reshape.default"} or len(source.parents) != 1:
        return None
    value_bmm = source.parents[0]
