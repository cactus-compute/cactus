...
def match_parent_tensor_dim_equal(source: models.Node, graph: models.Graph, fusion: FModels.FusionGraph, bindings: dict[str, models.Node], spec: dict[str, Any]) -> bool:
    ...
    left_parent = em.get_bound_parent(bindings, spec["left_node"], spec["left_parent_index"])
    right_parent = em.get_bound_parent(bindings, spec["right_node"], spec["right_parent_index"])
    if left_parent is None or right_parent is None:
        return bool(spec.get("allow_missing", False))
    left_dim = em.get_node_dim(left_parent, spec["left_dim"])
    right_dim = em.get_node_dim(right_parent, spec["right_dim"])
...
def match_input_broadcastable_to_node(source: models.Node, graph: models.Graph, fusion: FModels.FusionGraph, bindings: dict[str, models.Node], spec: dict[str, Any]) -> bool:
    ...
    if not input_shape or not target_shape:
        return bool(spec.get("allow_missing", False))
    return em.shapes_are_broadcastable(input_shape, target_shape)
...
def match_same_layer(source: models.Node, graph: models.Graph, fusion: FModels.FusionGraph, bindings: dict[str, models.Node], spec: dict[str, Any]) -> bool:
    ...
    for node_name in spec["nodes"]:
        ...
        if node is None:
            return False
        layer_key = em.get_layer_key(node)
...
def match_slice_halves(source: models.Node, graph: models.Graph, fusion: FModels.FusionGraph, bindings: dict[str, models.Node], spec: dict[str, Any]) -> bool:
    ...
    if not source_shape or not left_shape or not right_shape:
        return bool(spec.get("allow_missing", False))
    left_axis = em.normalized_slice_axis(left, len(source_shape))
    right_axis = em.normalized_slice_axis(right, len(source_shape))
    if left_axis is match_utils.MISSING or right_axis is match_utils.MISSING or left_axis != right_axis:
        return False
    if not em.slice_step_is_one(left) or not em.slice_step_is_one(right):
        return False
    ...
    if not match_utils.values_equal(left_size, right_size):
        return False
    if not em.attr_matches(left, "start", 0, default=0):
        return False
    if not em.attr_matches(right, "start", left_size):
        return False
    if not em.optional_attr_matches(left, "end", left_size):
        return False
    if not em.optional_attr_matches(right, "end", em.add_dims(left_size, right_size)):
        return False
    if source_size is not None and not match_utils.values_equal(source_size, em.add_dims(left_size, right_size)):
        return False
    return True
