...
def runtime_plan_metadata_from_model_profile(model_profile: Any | None) -> dict[str, str]:
    ...
    put_contract_metadata(
        metadata, model_profile.prompt_contract, "prompt_",
        csv=("suppress_generation_token_ids",),
        aliases={
            "repetition_penalty_scope": "repetition_penalty_scope",
            "suppress_generation_token_ids": "suppress_generation_token_ids",
        },
    )
    put_contract_metadata(
        metadata, model_profile.media_contract, "media_",
        csv=(
            "media_order", "image_focus_keywords", "audio_focus_keywords",
            "chunk_prefill_modalities", "image_feature_names", "audio_feature_names",
        ),
        pairs=("chunk_output_sources",),
        aliases={"media_order": "media_order"},
    )
    if model_profile.media_contract.injection_strategy:
        metadata["media_placeholder_token_id"] = str(model_profile.media_contract.placeholder_token_id)
    put_contract_metadata(
        metadata, model_profile.cache_contract, "cache_",
        csv=("fp16_kv_cache_components",),
        exclude=("full_retention_kv_layers",),
        aliases={
            "max_cache_sequence_length": "cache_max_sequence_length",
            "fp16_kv_cache_components": "fp16_kv_cache_components",
        },
    )
    put_contract_metadata(
        metadata, model_profile.runtime_contract, "runtime_",
        exclude=("states", "aliases"),
    )
    return metadata

def put_contract_metadata(
    metadata: dict[str, str],
    contract: Any,
    prefix: str,
    *,
    ...
    for contract_field in fields(contract):
        ...
        if name in exclude:
            continue
        key = aliases.get(name, f"{prefix}{name}")
        value = getattr(contract, name)
        if name in csv:
            put_csv(metadata, key, value)
        elif name in pairs:
            put_pair_csv(metadata, key, value)
        else:
            put_if_present(metadata, key, value)
