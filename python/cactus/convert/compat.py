...
_GEMMA4_MODULE = "transformers.models.gemma4.modeling_gemma4"
...
def _ensure_transformers_supports_gemma4() -> str | None:
    if _transformers_supports_model_module(_GEMMA4_MODULE):
        return None

    for site_packages in _candidate_external_site_packages():
        candidate = site_packages / Path(*_GEMMA4_MODULE.split("."))
...
def _patch_transformers_torchvision_probe() -> str | None:
    ...
    if has_lzma:
        return _patch_torchvision_missing_nms_op()
...
def patch_transformers_import_compat() -> list[str]:
    ...
    for patch in (
        ...
        _ensure_transformers_supports_gemma4,
    ):
        try:
            note = patch()
        except Exception:
