...
struct TQVarianceDump {
    ...
    std::map<size_t, PerHd> by_hd;
    bool enabled() {
        static bool e = []() {
            bool on = (std::getenv("CACTUS_TQ_DUMP_VARIANCE") != nullptr);
            if (on) {
                std::fprintf(stderr, "[TQ variance dump] enabled\n");
                std::atexit([]() { tq_var_dump().dump(); });
            }
            return on;
        }();
        return e;
    }
    void accumulate(const float* pre, const float* post, size_t hd) {
        if (!enabled()) return;
        std::lock_guard<std::mutex> lock(mu);
        auto& s = by_hd[hd];
        if (s.pre_sum_sq.empty()) {
            s.pre_sum_sq.assign(hd, 0.0);
            s.post_sum_sq.assign(hd, 0.0);
            s.post_max_abs.assign(hd, 0.0);
            s.mean.assign(hd, 0.0);
            s.M2.assign(hd, 0.0);
            s.M3.assign(hd, 0.0);
            s.M4.assign(hd, 0.0);
            s.reservoir.assign(hd, {});
            for (auto& r : s.reservoir) r.reserve(PerHd::RESERVOIR_K);
        }
        s.n_samples++;
        ...
        for (size_t i = 0; i < hd; i++) {
            double pv = static_cast<double>(pre[i]);
            double qv = static_cast<double>(post[i]);
            s.pre_sum_sq[i] += pv * pv;
            s.post_sum_sq[i] += qv * qv;
            double a = std::abs(qv);
            ...
            double term1 = delta * delta_n * static_cast<double>(n - 1);
            s.mean[i] += delta_n;
            s.M4[i] += term1 * delta_n2 * (static_cast<double>(n) * static_cast<double>(n) - 3.0 * static_cast<double>(n) + 3.0)
                     + 6.0 * delta_n2 * s.M2[i] - 4.0 * delta_n * s.M3[i];
            ...
            if (s.reservoir[i].size() < PerHd::RESERVOIR_K) {
                ...
                uint64_t r = next_rand() % n;
                if (r < PerHd::RESERVOIR_K) s.reservoir[i][r] = static_cast<float>(qv);
            }
        }
    }
    void dump_one(size_t hd, const PerHd& s, FILE* csv) {
        if (s.n_samples == 0) return;
        std::vector<double> pre_var(hd), post_var(hd);
        for (size_t i = 0; i < hd; i++) {
            pre_var[i] = s.pre_sum_sq[i] / static_cast<double>(s.n_samples);
            post_var[i] = s.post_sum_sq[i] / static_cast<double>(s.n_samples);
        }
        auto stats = [](std::vector<double> v) {
            std::sort(v.begin(), v.end());
            double med = v[v.size() / 2];
            double p99 = v[std::min(v.size() - 1, static_cast<size_t>(0.99 * v.size()))];
            double mn = v.front(), mx = v.back();
            return std::array<double, 5>{med, p99, mn, mx, mx / std::max(1e-30, med)};
        };
        auto pre_s = stats(pre_var);
        auto post_s = stats(post_var);
        std::fprintf(stderr,
            "\n[TQ variance dump] head_dim=%zu samples=%llu\n",
            hd, static_cast<unsigned long long>(s.n_samples));
        std::fprintf(stderr,
            "  pre-rotation:  median=%.4g p99=%.4g min=%.4g max=%.4g  max/median=%.2f\n",
            pre_s[0], pre_s[1], pre_s[2], pre_s[3], pre_s[4]);
        std::fprintf(stderr,
        ...
        for (size_t M : {4, 8, 16, 32}) {
            std::fprintf(stderr, "[TQ adaptive outliers] hd=%zu top%zu=", hd, M);
            for (size_t i = 0; i < M && i < ranked.size(); i++) {
                std::fprintf(stderr, "%s%zu", (i == 0 ? "" : ","), ranked[i].second);
            }
            std::fprintf(stderr, "\n");
        }
        ...
        if (csv) {
            ...
            for (size_t i = 0; i < hd; i++) {
                double mean = s.mean[i];
                double variance = (n > 1) ? s.M2[i] / (n - 1) : 0.0;
                double skewness = 0.0, kurtosis = 0.0;
                if (s.M2[i] > 1e-30) {
                    skewness = (n > 0) ? std::sqrt(n) * s.M3[i] / std::pow(s.M2[i], 1.5) : 0.0;
                ...
                auto pct = [&](double q) -> double {
                    ...
                    size_t k = std::min(r.size() - 1, static_cast<size_t>(q * r.size()));
                    return static_cast<double>(r[k]);
                };

                std::fprintf(csv,
                    "%zu,%zu,%llu,%.6g,%.6g,%.6g,%.6g,%.6g,%.6g,%.6g,%.6g,%.6g,%.6g\n",
                    hd, i, static_cast<unsigned long long>(s.n_samples),
                    mean, variance, std::sqrt(std::max(0.0, variance)),
                    s.post_max_abs[i],
                    skewness, kurtosis,
                    pct(0.01), pct(0.50), pct(0.90), pct(0.99), pct(0.999));
            }
        }
    }
    void dump() {
        ...
        if (const char* path = std::getenv("CACTUS_TQ_DUMP_CSV")) {
            ...
            if (csv) {
                std::fprintf(csv,
                    "head_dim,channel,n_samples,mean,variance,std,max_abs,skewness,kurtosis,p01,p50,p90,p99,p99_9\n");
            }
        }
        for (auto& kv : by_hd) dump_one(kv.first, kv.second, csv);
...
struct AdaptiveOutliers {
    ...
    AdaptiveOutliers() {
        for (size_t hd : {size_t(64), size_t(128), size_t(256), size_t(512)}) {
            char env_name[64];
            std::snprintf(env_name, sizeof(env_name), "CACTUS_TQ_ADAPTIVE_HD%zu", hd);
            const char* val = std::getenv(env_name);
            if (!val) continue;
            std::vector<bool> m(hd, false);
            std::string s(val);
            size_t pos = 0;
            while (pos < s.size()) {
                ...
                if (!tok.empty()) {
                    try {
                        size_t idx = std::stoul(tok);
                        if (idx < hd) m[idx] = true;
                    } catch (...) {}
                }
                pos = comma + 1;
            }
            ...
            std::fprintf(stderr, "[TQ adaptive] hd=%zu outliers=%zu (%.1f%%)\n",
                         hd, count, 100.0 * count / hd);
        }
    }

    static AdaptiveOutliers& get() {
        static AdaptiveOutliers a;
        return a;
    }

    const std::vector<bool>* lookup(size_t hd) {
        auto it = mask.find(hd);
        return it != mask.end() ? &it->second : nullptr;
    }
};
...
static inline void apply_adaptive_simulation(float* buf, size_t head_dim) {
    ...
    if (!mask) return;
    const float sqrt_dim = sqrtf(static_cast<float>(head_dim));
    for (size_t c = 0; c < head_dim; c++) {
        if (!(*mask)[c]) {
            buf[c] = snap_to_4bit_unit(buf[c], sqrt_dim);
        }
    }
}
...
void cactus_turboquant_encode_kv_fp16(
    ...
    const bool dump = tq_var_dump().enabled();
