#pragma once
// Parses the on-disk .weights format into a CactusTQMatrix ready for kernel calls.
// All pointed-to data is owned by TQWeightData; the CactusTQMatrix holds raw pointers into it.
...
// Loads up to max_rows rows from path.  Returns false on any parse error.
inline bool tq_load_weights(const char* path, uint32_t max_rows, TQWeightData& out) {
    ...
    if (r32(12) != 2) return false;              // ndim must be 2
    uint64_t dim0       = r64(16);
    uint64_t dim1       = r64(24);
    uint32_t bits       = r32(76);
    uint32_t group_size = r32(68);
    uint32_t num_groups = r32(72);
    if (r32(128) != 0) return false;             // rotation_family must be hadamard
    if (r32(132) == 0) return false;             // must have input_scale
    ...
    if (source_flags & FLAG_PANEL_MAJOR) return false;   // panel-major not supported here
    ...
    // rotation (left_signs, right_signs, permutation)
