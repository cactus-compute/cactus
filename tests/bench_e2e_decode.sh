...
Options:
  ...
  --mtp-max-drafts L  Comma-separated draft run list (default: 0,2)
...
long_context_brief = " ".join([
    ...
    "Option B ships MTP behind an adaptive gate that starts with one draft token and disables itself after repeated rejections.",
    ...
    "The same engineer warned that a bad adaptive gate can hide regressions by silently turning MTP off, so dashboards must report both enabled time and accepted-token rate.",
...
for entry in "${MODELS[@]}"; do
    ...
    for idx in "${!PROMPTS[@]}"; do
        ...
        for draft in "${MTP_DRAFT_VALUES[@]}"; do
            mtp_flags=()
            if [[ "$MTP_FIXED_DRAFT" == "1" ]]; then
                mtp_flags+=("--mtp-fixed-draft")
            fi
