...
By default the CSV includes only the baseline (`0`) run with 2 warmups and 5 measured reps. Use `--mtp-max-drafts 0,2,3,4` for sweeps, or `--mtp-max-draft 3` for one setting. Sampled runs can be measured with `--temperature`, `--top-p`, `--top-k`, `--min-p`, and `--seed`.
