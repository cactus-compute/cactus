#include "engine.h"
#include "src/deepseek_v4_model.h"
...
int main(int argc, char** argv) {
    ...
    try {
        for (int i = 1; i < argc; ++i) {
            ...
            if (arg == "--model") model_dir = need_value("--model");
            else if (arg == "--ids") ids_csv = need_value("--ids");
            else if (arg == "--max-new-tokens") max_new_tokens = static_cast<size_t>(std::stoul(need_value("--max-new-tokens")));
            else if (arg == "--context") context_size = static_cast<size_t>(std::stoul(need_value("--context")));
            else if (arg == "--temperature") temperature = std::stof(need_value("--temperature"));
            else if (arg == "--top-p") top_p = std::stof(need_value("--top-p"));
            else if (arg == "--top-k") top_k = static_cast<size_t>(std::stoul(need_value("--top-k")));
            else if (arg == "--stop-ids") stop_ids_csv = need_value("--stop-ids");
            else if (arg == "--profile-prefix") profile_prefix = need_value("--profile-prefix");
            else if (arg == "--warmup-moe-experts") warmup_moe_experts = true;
        ...
        if (prompt_ids.empty()) throw std::runtime_error("--ids must contain at least one token id");
        std::unordered_set<uint32_t> stop_ids;
        for (uint32_t id : parse_ids(stop_ids_csv)) stop_ids.insert(id);

        cactus::engine::DeepSeekV4Model model;
        ...
        auto first_end = generation_start;
        if (max_new_tokens > 0) {
