...
void print_usage(const char* argv0) {
    std::cerr << "Usage: " << argv0
              << " <model_path> [--backend cpu|metal|vulkan] [--system <prompt>] [--image <path>] [--audio <path>]"
...
int main(int argc, char** argv) {
    ...
    if (backend == "cpu" || backend == "metal" || backend == "vulkan") {
        if (cactus_set_backend(backend.c_str()) == 0)
            std::cout << "Backend: " << (backend == "cpu" ? "CPU" : backend == "metal" ? "Metal GPU" : "Vulkan GPU") << "\n";
        else
            std::cout << "GPU not available; using CPU\n";
    } else if (!backend.empty()) {
        std::cerr << "Error: unknown backend '" << backend << "' (expected 'cpu', 'metal', or 'vulkan')\n";
        return 1;
    }
