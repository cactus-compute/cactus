  ...
  factory CompletionResult.fromJson(Map<String, dynamic> json) {
    return CompletionResult(
      ...
      functionCalls: json["function_calls"] != null
          ...
          : null,
      promptTokens: json["prefill_tokens"] ?? json["prompt_tokens"] ?? 0,
      completionTokens: json["decode_tokens"] ?? json["completion_tokens"] ?? 0,
      timeToFirstToken: ((json["time_to_first_token_ms"] ?? json["time_to_first_token"] ?? 0.0) as num).toDouble() / 1000.0,
      totalTime: ((json["total_time_ms"] ?? json["total_time"] ?? 0.0) as num).toDouble() / 1000.0,
      prefillTokensPerSecond: ((json["prefill_tps"] ?? json["prefill_tokens_per_second"] ?? 0.0) as num).toDouble(),
      decodeTokensPerSecond: ((json["decode_tps"] ?? json["decode_tokens_per_second"] ?? 0.0) as num).toDouble(),
      confidence: ((json["confidence"] ?? 1.0) as num).toDouble(),
      needsCloudHandoff: json["cloud_handoff"] ?? json["needs_cloud_handoff"] ?? false,
    );
  }
}
...
class Cactus {
  ...
  static Cactus create(String modelPath, {String? corpusDir}) {
    ...
    try {
      final handle = _cactusInit(modelPathPtr, corpusDirPtr, false);
      if (handle == nullptr) {
  ...
  CompletionResult completeMessages(
    ...
    try {
      ...
      if (result < 0) {
        final errBuf = responseBuffer.cast<Utf8>().toDartString(); throw CactusException('Completion failed: ${getLastError()} | buffer: $errBuf');
      }
